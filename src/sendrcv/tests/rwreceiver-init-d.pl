...
while (<SRC>) {
    ...
    if (/^(POST_COMMAND=).*/) {
        $daemon_conf_text .= $1 . "'sender \%I sent file \%s'" . "\n";
        next;
    }
    if (/^(DUPLICATE_DIRS=)/) {
        $skip = 1;
        $daemon_conf_text .= $_ ."\n";
        $daemon_conf_text .= "  $tmpdir/duplicate-dir1\n";
        $daemon_conf_text .= "  $tmpdir/duplicate-dir2\n";
        next;
    }
    if (/^(SENDER_SERVERS=)/) {
        $skip = 1;
        $daemon_conf_text .= $_ ."\n";
        $daemon_conf_text .= "  sender-1  $host:$port\n";
        next;
    }
    if ($skip) {
